...
class TorchModelTrainer:
    def __init__(self, model: torch.nn.Module, epochs: int, batch_size: int):
        ...
        self._scheduler = torch.optim.lr_scheduler.StepLR(self._optimizer, 1, gamma=0.95)
    ...
    def _batchify(self, matching_pairs, embedding_lookup):
        def make_batch_data(cur_batch):
            ...
            X = [[embedding_lookup[e1], embedding_lookup[e2]] for e1, e2, _ in pair_labels]
            Y = [label for _, _, label in pair_labels]
            return X, Y

        return [make_batch_data(i)
                for i in range(0, len(matching_pairs) - 1, self._batch_size)]

    def fit(self, labelled_pairs: List[Tuple[str, str, int]], embedding_lookup: Dict[str, np.array]):
